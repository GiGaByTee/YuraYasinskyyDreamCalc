...
- (UIImage *)blurWithCoreImage:(UIImage *)sourceImage
  ...
  CGContextSetFillColorWithColor(
      outputContext, [UIColor colorWithWhite:1 alpha:0.001].CGColor);
...
- (UIImage *)screenshotTaker:(UIView *)viewToTakeScreenshot {
  ...
  if ([[UIScreen mainScreen] respondsToSelector:@selector(scale)]) {
    UIGraphicsBeginImageContextWithOptions(viewToTakeScreenshot.bounds.size, NO,
...
- (void)positioningObjectsInCircle:(NSArray *)buttonsArray
                 andPreferedRadius:(CGFloat)radius
                   andScreenBounds:(CGRect *)bounds
                 andVerticalOffset:(NSInteger)offset {
  CGFloat rX, rY, c, s;
...
- (void)animationsMaker:(UIView *)myView

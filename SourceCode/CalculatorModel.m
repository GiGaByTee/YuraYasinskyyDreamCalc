...
- (NSString*)regexSyntaxCheker:(NSString*)inputText {
  ...
  NSString* pattern = @"(?:\\*|\\/|\\+|\\-|\\.|\\^)[\\*\\/"
                      @"\\+\\-\\.\\^]|\\([\\)\\*\\/\\+\\-\\.\\^]|(?:\\*|\\/"
                      @"|\\+|\\-|\\.\\^)[\\)]|\\)[\\.\\(]|xx|\\(\\d{1,10}\\)"
                      @"\\d{1,10}|\\d{1,10}\\(\\d{1,10}\\)|\\d{1,10}\\.\\d{1,"
                      @"10}\\.|\\d{1,10}\\/0{1,10}\\d{1,10}|[\\*\\/"
                      @"\\+\\-\\.\\^]$";
  ...
  NSArray* matches = [regex matchesInString:myEquation
                                    options:NSMatchingWithoutAnchoringBounds
                                      range:NSMakeRange(0, myEquation.length)];
  if (matches.count != 0) {
...
- (void)functionTabulator:(NSString*)inputStringFunction
  ...
  for (CGFloat i = rangeStart; i <= rangeEnd; i += step) {
    functionToTabulate = [inputStringFunction
        stringByReplacingOccurrencesOfString:@"x"
                                  withString:[NSString
                                                 stringWithFormat:@"%g", i]];
    [xData addObject:[NSNumber numberWithDouble:i]];  // x values

    resultYforX = [NSNumber
        numberWithDouble:
            [self calculatingResultFromRPN:
                      [self reversePolishNotation:
                                [self prepareToRPN:functionToTabulate]]]];
...
// For example labelInput = @"77.0+(55-2)*4"; after parsing should be 77.0 + (
// 55 - 2 ) * 4
- (NSString*)parseUserInputFromLabel:(NSString*)inputText {
  ...
  for (NSInteger i = 0; i < labelInput.length; i++) {
    if ([[NSString stringWithFormat:@"%c", [labelInput characterAtIndex:i]]
            isEqualToString:@"("] ||
        [[NSString stringWithFormat:@"%c", [labelInput characterAtIndex:i]]
            isEqualToString:@")"] ||
        [[NSString stringWithFormat:@"%c", [labelInput characterAtIndex:i]]
            isEqualToString:@"+"] ||
        [[NSString stringWithFormat:@"%c", [labelInput characterAtIndex:i]]
      ...
      if (i == 0) {
        ...
        parsedString =
            [NSString stringWithFormat:@"%@%c", parsedString,
                                       [labelInput characterAtIndex:i]];
      ...
      if (i == 0) {
        ...
        if ([[NSString
                stringWithFormat:@"%c", [labelInput characterAtIndex:i - 1]]
                isEqual:@"("] ||
            [[NSString
                stringWithFormat:@"%c", [labelInput characterAtIndex:i - 1]]
                isEqual:@")"] ||
...
- (CGFloat)resultWithNSExpression:(NSString*)inputString {
  ...
  NSExpression* expression =
      [NSExpression expressionWithFormat:numericExpression];
...
// Separating numbers and operators from input string and putting them into
// NSArray
- (NSArray*)prepareToRPN:(NSString*)inputString {
  NSString* stringToProcess =
      inputString;  // input string that will be proccessed
  ...
  for (NSInteger i = 0; i < stringToProcess.length; i++) {
    if ([[NSString stringWithFormat:@"%c", [stringToProcess characterAtIndex:i]]
            isEqual:@" "]) {
      ...
      tempString =
          [NSString stringWithFormat:@"%@%c", tempString,
                                     [stringToProcess characterAtIndex:i]];
    }
  }
...
- (NSArray*)reversePolishNotation:(NSArray*)preparedArray {
  ...
  NSMutableArray* rpnOutputString =
      [[NSMutableArray alloc] init];  // output string
  ...
  [numberFormatter setDecimalSeparator:@"."];
    ...
    BOOL unaryOperation = NO;

  for (NSInteger i = 0; i < dataPreparedToRPN.count; i++) {
      
      
      if ((i == 0) && ([[dataPreparedToRPN objectAtIndex:i] isEqual:@"-"] || [[dataPreparedToRPN objectAtIndex:i] isEqual:@"+"])) {
          tempString =
          [NSString stringWithFormat:@"%@%@", [dataPreparedToRPN objectAtIndex:i], [dataPreparedToRPN objectAtIndex:i+1]];
          numberFromPreparedToRPN =
          [[numberFormatter numberFromString:tempString] doubleValue];
          NSLog(@"temp string %@", tempString);
          
          [rpnOutputString
           addObject:[NSNumber numberWithDouble:numberFromPreparedToRPN]];
          NSLog(@"number recognized: %f", numberFromPreparedToRPN);
          
          unaryOperation = YES;
          NSLog(@"Unary operation detected.");
          continue;
      }
      else {
          
      if ((unaryOperation == YES) && (i == 1)) {
    ...
    tempString =
        [NSString stringWithFormat:@"%@", [dataPreparedToRPN objectAtIndex:i]];
    numberFromPreparedToRPN =
        [[numberFormatter numberFromString:tempString] doubleValue];
    ...
    if (numberFromPreparedToRPN == 0) {
      if ([tempString isEqual:@"^"]) {
        ...
        while ([rpnResultStack.lastObject isEqual:@"*"] ||
               [rpnResultStack.lastObject isEqual:@"/"] ||
               [rpnResultStack.lastObject isEqual:@"^"]) {
          [rpnOutputString addObject:rpnResultStack.lastObject];
          [rpnResultStack removeLastObject];
        }
        ...
        while ([rpnResultStack.lastObject isEqual:@"*"] ||
               ...
               [rpnResultStack.lastObject isEqual:@"+"] ||
               [rpnResultStack.lastObject isEqual:@"-"] ||
               [rpnResultStack.lastObject isEqual:@"sin"] ||
               [rpnResultStack.lastObject isEqual:@"cos"] ||
               [rpnResultStack.lastObject isEqual:@"tan"] ||
               [rpnResultStack.lastObject isEqual:@"ctg"] ||
               [rpnResultStack.lastObject isEqual:@"^"]) {
          [rpnOutputString addObject:rpnResultStack.lastObject];
          [rpnResultStack removeLastObject];
        }
...
- (CGFloat)calculatingResultFromRPN:(NSArray*)rpnOutputString {
  ...
  for (NSInteger i = 0; i < dataAfterRPN.count; i++) {
    if ([[dataAfterRPN objectAtIndex:i] isEqual:@"^"]) {
      tempNumber = pow(
          [[resultStack objectAtIndex:(resultStack.count - 2)] doubleValue],
          [[resultStack objectAtIndex:(resultStack.count - 1)] doubleValue]);
      ...
      tempNumber = pow(tan(([[resultStack objectAtIndex:(resultStack.count - 1)]
                           doubleValue])),
                       -1);
      [resultStack removeLastObject];
      [resultStack addObject:[NSNumber numberWithDouble:tempNumber]];

...
- (void)drawRect:(CGRect)rect {
  ...
  CGContextSetStrokeColorWithColor(
      context,
      [[UIColor colorWithRed:0.98 green:0.98 blue:1.00 alpha:1.0] CGColor]);
  ...
  for (NSInteger i = 0; i <= howManyHorizontal; i++) {
    if (i == howManyHorizontal / 2) {
      ...
      CGContextAddLineToPoint(context, defaultGraphWidth,
                              graphBottom - offsetY - i * stepY);
      CGContextStrokePath(context);
      CGContextBeginPath(context);
      CGContextSetStrokeColorWithColor(
          context,
  ...
  CGContextSetFillColorWithColor(
      context, [[UIColor colorWithRed:0 green:0 blue:0 alpha:1.0] CGColor]);
  for (NSInteger i = 0; i < self.countOfElements; i++) {
    if (i == ((defaultGraphWidth - offsetX) / stepX) / 2) {
      ...
      [labelsText drawAtPoint:CGPointMake((offsetX + i * stepX) + 3,
                                          graphBottom - defaultGraphHeight / 2)
               withAttributes:0];
    }
  }
}
...
- (void)drawLineGraphWithContext:(CGContextRef)context {
  ...
  for (NSInteger i = 0; i < self.countOfElements; i++) {
    if ([[self.transferedDataY objectAtIndex:i]
            isEqual:[NSNumber numberWithDouble:INFINITY]] ||
        [[self.transferedDataY objectAtIndex:i]
            isEqual:[NSNumber numberWithDouble:NAN]]) {
      continue;
    }

    CGContextMoveToPoint(
        context,
        (startX + [[self.transferedDataX objectAtIndex:i] doubleValue] * scale),
        (startY -
         [[self.transferedDataY objectAtIndex:i] doubleValue] * scale));
    if (i == self.countOfElements - 1) {
      ...
      CGContextAddLineToPoint(
          context,
          (startX +
           [[self.transferedDataX objectAtIndex:i + 1] doubleValue] * scale),
          (startY -
           [[self.transferedDataY objectAtIndex:i + 1] doubleValue] * scale));
      NSLog(@"dataX[%li]: %g, dataY[%li]: %g", (long)i,
            [[self.transferedDataX objectAtIndex:i] doubleValue], (long)i,
            [[self.transferedDataY objectAtIndex:i] doubleValue]);
    }
  }
